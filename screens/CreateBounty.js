...
import { Button, ButtonGroup, Datepicker, Icon, Input, Text, Toggle, Divider, Radio, Modal, IndexPath, Select, SelectItem } from '@ui-kitten/components';
import * as ImagePicker from 'expo-image-picker';

const Tab = createBottomTabNavigator();

const BountyPage = ({navigation}) => {
...
const WantePage = () => {
  ...
  >
    {
      (function(){
        ...
        for(let i = 0; i < themes.length; i++){
          arr.push(<SelectItem title={themes[i]}/>)
        }
        return arr;
      })()
    }
  ...
  {
    (function(){
      if(themes[selectedIndex.row] == "Other..."){
        return(
          <Input
            placeholder='Other Theme Here'
            style={styles.input}
            multiline={true}
            value={who}
...
function CreateBounty(){
  ...
  <Tab.Navigator tabBarOptions={{ showLabel: false}}>
    <Tab.Screen name="Bounty" component={BountyPage} options={{ tabBarIcon: ({ focused }) => (<View style={{alignItems:'center',justifyContent:'center'}}><Image source={require("../images/home.png")} resizeMode='contain' style={{width:25,height:25,tintColor:focused?"#e32f45":"#748c94"}}></Image><Text style={{color:focused?"#e32f45":'#748c94',fontSize:12}}>Bounty</Text></View>) }} />
    <Tab.Screen name="Wante" component={WantePage} options={{ tabBarIcon: ({ focused }) => (<View style={{alignItems:'center',justifyContent:'center'}}><Image source={require("../images/home.png")} resizeMode='contain' style={{width:25,height:25,tintColor:focused?"#e32f45":"#748c94"}}></Image><Text style={{color:focused?"#e32f45":'#748c94',fontSize:12}}>Wante</Text></View>) }} />
  </Tab.Navigator>
  )
}
